...
Before proving the theorem, we first discuss several lemmas.
...
\begin{lemma}[{\cite[Chap.~1 Lemma 6.2.1]{hazewinkel1978formal}}]
    ...
    $R$. Then there exists a nonzero homomorphism from $F(X,Y)$ into $\bfG_a(X,Y)$ or into
    $\bfG_m(X,Y)$.
...
\begin{proof}[Proof of Commutativity theorem \ref{comm_thm}]
    ...
    Let $0 \neq a \in R$ be an element of $R$ that is nilpotent and $\ZZ$-torsion. Then there exists
    ...
    from computations involving the associativity condition of a formal group law.
    A proof is given later in Corollary \ref{cor: torsion_free_comm}.
