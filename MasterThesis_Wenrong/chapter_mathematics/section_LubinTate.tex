...
elements in $\lambda \in \gothm_{\Fsep{K}}$ such that $\pi ^n \lambda = 0$.
...
\begin{proof}[Proof of theorem $\ref{thm: Lam_iso_K_quot_O}$]
    ...
    $\pi X + X ^ q = \alpha$ is separable, then $\Lambda_{f,1} \subset \gothm_{\Fsep{K}}$. This means
    ...
    $r M_f(\Fsep{K}) = M_f (\Fsep{K})$ which means that $M_f(\Fsep{K})$ is divisible $\calO_K$-module.
    ...
    It follows from the fact that $|J| = \dim_k M_f(\Fsep{K})[\pi] = \dim_k \Lambda_{f,1} = 1$,
...
\begin{proof}
    ...
    \[ \Gal(\Fsep{K}/K) \cong \varprojlim \Gal(L_{\pi,n}/ K) \cong \varprojlim U_K / U_K^n \cong U_K. \]
\end{proof}
...
independent of the choice of the uniformizer $\pi$.
...
\begin{proof}
    ...
    \[
        ...
        \pmod{\deg(n+2)}.
    \]
    Passing to the coefficientwise limit, we obtain a power series
    ...
    \]
    Then $F(X,Y)\equiv X+Y \pmod{\deg 2}$. Moreover, using
...
\begin{theorem}
    The field $L_\pi$ is independent of the choice of the uniformizer $\pi$.
\end{theorem}
...
\begin{theorem}
    The homomorphism $r_\pi$ is independent of the choice of the uniformizer
    $\pi$.
\end{theorem}
