...
\begin{lemma}[{\cite[Chap.~6 Proposition 5]{cassels1967algebraic}}]
    ...
    \begin{align}
        F (X_1, \ldots, X_n) &\equiv  L(X_1, \ldots, X_n) \pmod{\deg 2}, \\
...
\begin{proof}
    ...
    \begin{align*}
        F_k(X_1, \ldots , X_n) &\equiv L (X_1, \ldots, X_n) \pmod{\deg 2}, \\
    ...
    Now assume that the result holds for $k$. Write $f (F_k(X_1,\ldots, X_n )) \equiv
        F_k(g(X_1), \ldots, g(X_n)) + E_{k+1} \pmod{\deg (k+ 2)}$. According to the induction
...
\begin{proof}
    ...
    $G_2(X,Y,Z) = F_f(X,F_f(Y,Z))$, and we can check that $G_i(X,Y,Z) \equiv X+Y+Z \pmod{\deg 2}$ and
    \begin{equation*}
...
\begin{proof}
    Notice that $F_f([a]_{f,g}(X), [b]_{f,g}(X)) \equiv (a+b)X \pmod{\deg 2}$ and
    \begin{equation*}
