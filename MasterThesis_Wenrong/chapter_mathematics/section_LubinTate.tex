...
\begin{proof}
    ...
    And the uniqueness of this proposition follows from equation \eqref{eq: LT_trunc_k} and the uniqueness in
    the claim.
...
According to the Remark $\ref{rem: A-module}$, $F_f$ defines a $\calO_K$-module structure on
$\gothm_{K^{sep}}$ and we denote this $\calO_K$-module as $M_f(\Ksep)$.
Then $\Lambda_{f,n}$ is those
...
\begin{proof}[Proof of theorem $\ref{thm: Lam_iso_K_quot_O}$]
    ...
    It follows from the fact that $|J| = \dim_k M_f(\Ksep)[\pi] = \dim_k \Lambda_{f,1} = 1$,
...
\begin{proof}
    ...
    We have an injection $\Gal(K_{\pi,n}/ K) \to U_K/ U _K^n$, where $U_K = 1 + \pi ^ n \calO_K$.
...
Let $\Kun$ be the maximal unramified extension of $K$. Since $K_\pi$ is a totally ramified extension
of $K$, $K_\pi$ and $\Kun$ are linearly disjoint. As a result, the Galois group
$\Gal(K_\pi \Kun/K)$ is the direct product of $\Gal(K_\pi/K)$ and $\Gal(\Kun/K)$.

Let $\pi \in \calO_K$ be a uniformizer and let $L_\pi$ be the field $K_\pi \Kun$. We define a group homomorphism
...
% Before we begin to prove this theorem, we will state a lemma and
% skip the proof of this lemma.
