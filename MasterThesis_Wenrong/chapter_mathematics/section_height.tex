...
\begin{definition}[Height of a homomorphism, {\cite[Chap.~4 Definition 7.1]{silverman2009arithmetic}}]
    Let $F, G$ be formal group laws over $k$, and let $f \in k[\![X]\!]$ be a homomorphism from
    $F(X,Y)$ to $G(X,Y)$. Then the \emph{height of the homomorphism} $f$, denoted by $\htt(f)$,
    is the largest number $h$ such that
    \[f(X) = g\left(X ^ {p ^ h}\right), \]
    for some power series $g(X)$ over $k$. If $f= 0$, then we set $\htt(f) = \infty$.
\end{definition}
...
\begin{proposition}[{\cite[Chap.~4 Proposition 7.2]{silverman2009arithmetic}}]
    ...
    \begin{enumerate}
        \item If $f'(0) = 0$, then $f(X) = f_1(X ^ p)$ for some $f_1(X) \in k[\![X]\!]$. That is,
            $\htt(f) \ge 1$.
        \item There is a natural number $m$ such that $f(X) = h\left(X^{p^m}\right)$ for some $h$ such that
            $h'(0) \neq 0$.
        \item Write $f(X) = g\left(X^ {p^{\htt(f)}}\right)$. Then $g'(0) \neq 0$.
    \end{enumerate}
...
\begin{proof}
    ...
    If $f'(0) = 0$, then by Part (1) of this proposition, we have that
    ...
    Then again by Part (1) of this proposition, we have that there is an $f_2(X) \in k[\![X]\!]$,
    ...
    of it is nonzero. Notice that the order $f_i$ decline after each step.
...
\begin{remark}
    Write $f(X) = \sum_{n=n_0}^{\infty} a_n X ^ n$, with $a_{n_0} \neq 0$.
    By Part (2) and (3) of the
    Proposition \ref{prop: equiv_def_height},
    $n_0$ is of the form $p ^ t$ for some natural number $t$.
    Then we have a alternative description of the height of a formal group
    homomorphism $f(X)$ from $F$ to $G$, that is the height of $f(X)$ can be
    defined to be the this $t$.
    \label{rem: alter_def_height}
\end{remark}

\begin{definition}[Height of a formal group]
...
\begin{proposition}
    Let $F, G$ be formal group laws over $k$. Assume that
    $\htt(F) \neq \htt(G)$, then the only homomorphism from $F$ to $G$, is the zero homomorphism.
\end{proposition}
...
\begin{proof}
    Denote $f (X) = f_1 \left(X ^ {p ^ {\htt (f)}}\right)$ and $g(X) = g_1 \left(X ^ {p ^ {\htt (g)}}\right)$. Then
    \[(g\circ f) (X) = g_1 \left(f_1 (X ^ {p ^ {\htt (f)}}) ^ {\htt (g)}\right)
        = g_1 \left(\tilde{f}_1(X ^ {\htt(f) + \htt(g)})\right), \]
    where $\tilde{f}_1 (X)$ is obtained from $f_1(X)$ raising each coefficient of
    $f_1(X)$ to the $p^{\htt(g)}$-th power.
\end{proof}

\begin{lemma}
    Let $R$ be a commutative ring, $I, J$ be two index sets and
    $F \in R[\![X_I]\!]$. Let $\{f_i\}_{i\in I}$ be a set of power series in
    $R[\![X_J]\!]$ with the property that the constant
    coefficient of each $f_i$ is nilpotent in $R$.
    Let $G(X_J)$ be the power series obtained by substituting
    $f_i$ into $F$.
    We denote the order of power series $f$ to be $o(f)$.
    Then we have that
    \[o(G(X_J)) \ge o(F) \cdot  \min_{i\in I}\{o(f_i)\}. \]
...
\begin{proof}
    Recall that for any power series $f(X)$, $n \le o(f)$ is equivalent to for all $j < n$, the $j$-th homogeneous component
    $f(X)$ is zero.

    Then it is enough to show that, for any $j$ less than $o(F) \cdot  \min_{i\in I}\{o(f_i)\}$,
    the $j$-th homogeneous
    component of $G$ is zero. However, the least possible nonzero homogeneous component in
    $G(X)$ is at least of degree the product of
    the order of $F$ and the least of the order of the $f_i$.
\end{proof}

\begin{proposition}[{\cite{hazewinkel1978formal}, Definition 18.3.2}]
    Let $F$ be a formal group law over $k$, and let $f$ and $g$ be two formal group
...
\begin{proof}
    First, we show that $F(f,g)$ is an endomorphism of $F$. This follows from
    ...
    \end{align*}
    By Lemma \ref{lem: order_subst} and the fact that order of $F(X,Y)$ is $1$,
    then we have that
    \[ o(F(f(X), g(X))) \ge o(F) \cdot \min\{o(f),o(g)\} = \min\{o(f),o(g)\}. \]
    Therefore,
...
\begin{remark}
    Let $i(X)$ be the additive inverse of the formal group law $F$, i.e. the power
    ...
    Proposition \ref{prop: height_mul}, we have that the set of all endomorphisms over $k$ of $F$ form a
    ring, which we denote as $\End_k(F)$. The multiplication is given by composition of power series and the addition
    of $f, g \in \End_k(F)$ is defined to be $F(f,g)$. The height function
    $\htt$ defines a valuation on the ring $\End_k(F)$.
\end{remark}

If $k$ is an algebraically closed field of characteristic $p$,
then the following result shows that formal group laws over $k$ are classified by
their heights.
...
Since the proof of this theorem depends on many auxiliary results, we do
