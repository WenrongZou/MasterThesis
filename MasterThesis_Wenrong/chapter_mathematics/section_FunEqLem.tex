...
Since $f (X) \equiv b_1 X \pmod{\deg 2}$ and $f^{-1} (X) \equiv b_1^ {-1} X \pmod{\deg 2}$.
Hence $F(X, Y) = X + Y \pmod{\deg 2}$, i.e. $F_1 (X, Y)$ has its coefficients in $R$.
We now proceed by induction to prove that $F_n(X, Y) \in R[\![X, Y]\!]$.

Suppose that $F_1(X, Y), \ldots , F_{n - 1} (X, Y) \in R [\![X, Y]\!]$. And we have
\begin{equation*}
    (F_1(X, Y) + \cdots F_{n- 1} (X, Y)) ^ r \equiv (F(X, Y))^r \pmod{\deg (n + 1)}.
\end{equation*}
...
\begin{equation}
    s_i \sigma ^i (a_n) F (X, Y)^{q ^ i n} \equiv s_i\sigma ^ i(a_n)
        (\sigma _* ^ i F (X ^ {q ^ i}, Y ^ {q ^ i})) ^ n \pmod{(R, \deg (n + 1))}
\end{equation}
...
\[b_1 F_n(X,Y) \equiv 0 \pmod{(R, \deg (n + 1))}\] and $b_1$ is invertible in $R$,
then we get the result.

\textit{Proof of part 2:} Write $G(X) = f_g^{-1}(f_{h}(X))$. Let $G_i(X)$ be the homogeneous
...
then, modulo $\pmod{(R, \deg (n + 1))}$, we have
\begin{align*}
    f_{h}(X) &= f(G(X)) \\
    ...
    &= g(G(X)) + f_{h}(X) - h(X)
\end{align*}
Then we have that $g(G(X)) \equiv h(X) \pmod{(R, \deg (n + 1))}$. And since

\[h (X) \equiv g(G(X)) \equiv b_1 G_n(X) \pmod{(R, \deg (n + 1))}. \]

Since $b_1$ is invertible, $G_n(X)$ has coefficients in $R$.

\textit{Proof of part 3:} Denote $\hat{f}(X) = f(h(X))$. By the functional
