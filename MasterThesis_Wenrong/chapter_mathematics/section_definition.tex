...
\begin{proof}[Proof of equivalence of the definition]
    ...
    This proves that $F(X,0) = X$. Also, $F(0,X)= X$ follows from a similar argument.
\end{proof}
...
Combining the result above, a formal group law $F(X,Y)$ defines a group structure on the set of multivariate power series in $R[\![X_I]\!]$ with zero constant coefficient, where $I$ is any finite index set.
...
\begin{lemma}
    ...
    if $a_1$ is a unit in $R$, where $\alpha (X) = a_1 X + a_2 X^2 + \cdots $.
\end{lemma}
...
\begin{proposition}
    For any natural number $n$, the $n$-series $[n](X)$ is a formal group homomorphism from $F(X,Y)$ to
