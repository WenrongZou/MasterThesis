...
\begin{example}
    ...
    These names describe how each power series mimics standard operations with the identity
    element placed at $0$. The \emph{additive formal group law},
    $\mathbb{G}_a(X,Y) = X + Y$, is simply standard addition.
    The \emph{multiplicative formal group law}, $\mathbb{G}_m(X,Y) = X + Y + XY$,
    is standard multiplication shifted from an identity of $1$ to $0$. If we define
    variables centered at $1$ as $U = 1+X$ and $V = 1+Y$, their standard product is
    $(1+X)(1+Y) = 1 + X + Y + XY$. Subtracting $1$ to recenter the identity back to $0$
    yields exactly the multiplicative law: $X + Y + XY$.
...
\begin{proof}
    ...
    The desired equality follows by composing both sides with $\beta (X)$ and using $\beta \circ \alpha = \id$.
...
\begin{proof}
    Assume that $[p](X) \neq 0$. Let $k$ be the order of $[p](X)$, i.e.,
    the smallest natural number $i$ such that the $i$-th coefficient of
    $[p](X)$ is nonzero, and let $a$ be this leading coefficient. It is enough to show that $k$ is of the form $p^n$ for
    some $n$.

    Since $[p](X)$ is an endomorphism of the formal group law $F$, we have
    \[ [p](F(X,Y)) = F([p](X), [p](Y)). \]

    Reducing both sides of the above equation modulo degree $k + 1$,
    we obtain the following congruences:
    \[
    \begin{aligned}
    [p](F(X,Y)) &\equiv a(X + Y)^k \pmod{\deg (k + 1)}, \\
    F([p](X), [p](Y)) &\equiv aX^k + aY^k \pmod{\deg (k + 1)}.
    \end{aligned}
    \]

    Comparing the two sides, we have $a(X + Y)^k \equiv a(X^k + Y^k) \pmod{\deg (k + 1)}$.
    Since $a \neq 0$ and $R$ has characteristic $p$, it follows that the intermediate binomial coefficients must vanish, which means:
    \begin{equation}
        \forall i \in [1, k - 1], \quad p \mid \binom{k}{i}.
        \label{eq: dvd_choose}
    \end{equation}
    ...
    If $k$ is not a power of $p$, then there exists a natural number $m > 1$
    such that $k = p^t m$ and $p \nmid m$. Taking $i = p^t$ in
    relation \eqref{eq: dvd_choose}, we have
    \[ p \mid \binom{p^t m}{p^t}. \]
    By Lucas's theorem, we have
    \[ \binom{p^t m}{p^t} \equiv \binom{m}{1} \equiv m \pmod{p}. \]

    This yields the contradiction that $m \equiv 0 \pmod{p}$, since we originally assumed $p \nmid m$. Therefore, $k$ must be a power of $p$.
\end{proof}

% \begin{proof}
%     Assume that $[p](X) \neq 0$. Let $k$ be the order of $[p](X)$, i.e.
%     the minimal natural number $i$ such that the $i$-th coefficient of
%     $[p](X)$ is nonzero. It is enough to show that $k$ is of form $p^n$ for
%     some $n$.

%     Since $[p](X)$ is an endomorphism of formal group law $F$, we have that
%     \[[p](F(X,Y)) = F([p](X), [p](Y)). \]

%     We module $\deg (k + 1)$ at the both side of the above equation.
%     Then we have the following equalities
%     \[
%     \begin{aligned}
%     [p] (F(X,Y)) &\equiv (X + Y) ^ k \pmod{\deg (k + 1)} \\
%     F([p](X), [p](Y)) &\equiv X ^ k + Y ^ k \pmod{\deg (k + 1)}.
%     \end{aligned}
%     \]

%     Then we have that $(X + Y) ^ k \equiv X ^ k + Y ^ k \pmod{\deg (k + 1)}$.
%     It follows that
%     \begin{equation}
%         \forall i \in [1, k - 1], \quad p \mid \binom{k}{i}.
%         \label{eq: dvd_choose}
...
%     Recall that Lucas theorem: let  $p$ be a prime number,
%     $a, m, n$ be natural number, then
%     \[\binom{p^ a m }{p ^ a n} \equiv \binom{m}{n} \pmod{p}.  \]

%     If $k$ is not a power of $p$, then there is a natural number $m$
%     greater than $1$
%     such that $k = p ^ t m$ and $p \nmid m$. Taking $i = p ^ t$ in
%     relation \eqref{eq: dvd_choose}, then we have that
%     \[p \mid \binom{p ^ t m}{ p ^ t}. \]
%     By Lucas theorem, we have that
%     \[\binom{p ^ t m}{p ^ t} \equiv \binom{m}{1} \equiv m \pmod{p}. \]

%     Then we obtain a contradiction that $m \equiv 0 \pmod{p}$ and $p \nmid m$.
