...
To begin, we formalize the data involved in the lemma as follows.
\begin{lstlisting}
...
number and $q$ can be written as $q = p ^ t$ with $t \neq 0$. Let $\sigma$ be a ring
...
To state the formal equation, we need APIs related to \lstinline|expand|,
which is an algebra homomorphism that expands a
...
\lstinline|substAlgHom| is an algebra homomorphism that substitutes multivariate
power series into a multivariate power series. \lstinline|HasSubst.X_pow| means that
taking a nonzero power of \lstinline|X| lies in the family of power series that
can be substituted.

We formalize the functional equation $\ref{equation: recurFunEq}$ as follows.
\begin{lstlisting}
...
The API \lstinline|inv_add_RecurFun| denotes the multivariate power series
...
def FormalGroup.InvAdd_RecurFun [UniformSpace K] [T2Space K]
