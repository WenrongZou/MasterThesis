...
In this section, we discuss the formalization of the functional equation lemma.
To begin with, we formalize the data involved in the functional equation lemma as follows.
\begin{lstlisting}
variable {K : Type*} [CommRing K] {R : Subring K} {I : Ideal R}
  {p t q : ℕ} [hp : Fact (Nat.Prime p)] (ht : t ≠ 0) (hq : q = p ^ t)
  (σ : K →+* K) (s : ℕ → K) {g : PowerSeries R}
\end{lstlisting}
Let $K$ be a commutative ring, $R$ a subring of $K$, and $I$ an
ideal of $R$. Let $p, q, t$ be natural numbers, where $p$ is a prime
number and $q$ can be written as $q = p ^ t$ with nonzero $t$. Let $\sigma$ be a ring
endomorphism of $K$, and let $s$ be a sequence of elements in $K$.

According to equation $\ref{eq: coeff_RecurFun}$, we can inductively define a
function \lstinline{RecurFunAux} using equation $\ref{eq: coeff_RecurFun}$ and then obtain a power series
...
To state the formalization of the functional equation, we need APIs related to \lstinline|expand|,
which is an algebraic homomorphism that expands a
(multivariate) power series by a nonzero factor $p$. We discuss these APIs first.
...
We formalize functional equation $\ref{equation: recurFunEq}$ as follows.
\begin{lstlisting}
...
The theorem \lstinline|coeff_one_RecurFun| states that the 1st coefficient of $g$
equals the 1st coefficient of $f_g(X)$. With the assumption
...
$F_g(X,Y)$ has coefficients in $R$. Moreover, this is a commutative formal group law by
...
are zero, then $f_{g'}^{-1} (f_g(X))$ has coefficients in $R$. The second part of the functional equation lemma
is formalized as follows. In the lemma below, \lstinline|f_g'| is the power series $f_{g'}(X)$ and \lstinline|G| is
$f_{g'}^{-1}(f_g(X))$.
...
The third part of the functional equation lemma states that there is a power series $\hat{h}$ over $R$
...
I briefly explain some details in Part Four of the functional equation lemma.
To begin with, \lstinline|α.coeff n - β.coeff n| is an element in $K$, since $\alpha$ is
a power series over $R$ and $\beta$ is a power series over $K$. Therefore, we need to formalize it
as an element in a set of $K$ rather than in an ideal of $R$.
Secondly, \lstinline|R.subtype '' ↑(I ^ r)| is a set of elements in $K$ that is the image of $I^r$
