...
Now we discuss how to obtain the group instance for
\lstinline|F.Point σ|.

The additive monoid structure bundles together the identity and
associativity laws.  The zero point is a left and right identity for the
addition defined by \lstinline|F|, and associativity of the formal group
law gives associativity of the addition on points:
\begin{lstlisting}
...
underlying power series.  The theorem \lstinline|F.assoc'| is the
pointwise form of the associativity condition of the formal group law: it
says that substituting three valid points into the two sides of the
associativity identity gives the same power series.  The field
\lstinline|nsmul := nsmulRec| tells Lean to use the usual recursive
definition of repeated addition by natural numbers.
...
A formal group homomorphism does more than relate the two underlying
power series: it induces an actual map between the groups of points
\lstinline|F.Point σ| and \lstinline|G.Point σ|.  Given a homomorphism
\lstinline|f : FormalGroupHom F G| and a point \lstinline|x : F.Point σ|,
we substitute the power series \lstinline|x.val| into the power series of
\lstinline|f| to obtain a point of \lstinline|G|:

\begin{lstlisting}
def FormalGroupHom.applyPoint (f : FormalGroupHom F G) (x : F.Point σ) :
    G.Point σ := ⟨f.toPowerSeries.subst x.val, ...⟩
\end{lstlisting}

To make this induced map convenient to use, we register
\lstinline|applyPoint| as a \lstinline|CoeFun| instance.  This lets us
apply a homomorphism \lstinline|f| directly to a point, writing
\lstinline|f x| instead of \lstinline|f.applyPoint x|:

\begin{lstlisting}
instance : CoeFun (FormalGroupHom F G)
    (fun _ ↦ {σ : Type*} → F.Point σ → G.Point σ) where
  coe f := f.applyPoint
\end{lstlisting}

The homomorphism condition is precisely the statement that this induced
map respects the formal group addition.  We therefore prove that it
preserves addition, and then bundle the map together with the proofs
that it preserves zero and addition into an \lstinline|AddMonoidHom|:
...
In this way, every formal group homomorphism gives rise to a genuine
homomorphism of additive monoids between the associated groups of points.
This allows us to reuse the entire Mathlib API for \lstinline|AddMonoidHom|
when reasoning about the map induced by a formal group homomorphism.
...
Just as a homomorphism induces a map on points, an isomorphism induces an
isomorphism between the groups of points.  Using the maps
\lstinline|applyPoint| of \lstinline|toHom| and \lstinline|invHom|
together with the two inverse conditions, we package a formal group
isomorphism into an \lstinline|AddEquiv| between \lstinline|F.Point σ|
and \lstinline|G.Point σ|:
...
Here the fields \lstinline|toFun| and \lstinline|invFun| are the maps on
points induced by \lstinline|toHom| and \lstinline|invHom|, while
\lstinline|left_inv| and \lstinline|right_inv| follow from the
\lstinline|left_inv| and \lstinline|right_inv| fields of the
isomorphism.  The field \lstinline|map_add'| reuses
\lstinline|FormalGroupHom.map_add| for \lstinline|toHom|.  Thus a formal
group isomorphism yields an isomorphism of additive groups
\lstinline|F.Point σ ≃+ G.Point σ|, and we may again invoke the Mathlib
API for \lstinline|AddEquiv|.
