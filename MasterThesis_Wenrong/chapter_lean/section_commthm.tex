...
In this section, we will discuss some formalization of the commutative result in the previous
chapter. Fix a formal group law $F(X,Y)$ over $R$ thoughout this section.
First, we fix some environmental variables.

\begin{lstlisting}
variable {R : Type*} [CommRing R] (F : FormalGroup R) [Nontrivial R] {σ : Type*}
\end{lstlisting}

According to the definition of commutativity of formal group law, we have that $F(X,Y)$ is
...
$x y x^{-1} y ^{-1}$ where $x, y$ are elements of group $G$ and a group is commutative if and only
if its commutator is zero.
A formal group law $F(X,Y)$ is commutative if and only if the precommutator equals to
\lstinline|Y|.
...
Now we could formalize the lemma \ref{lem: non_com_hom} and lemma \ref{lem: target_com}.
...
The instance \lstinline|IsDomain R| means \lstinline|R| is an integral domain.
Here \lstinline|Gₐ|, \lstinline|Gₘ| are the abbreviation of the additive and multiplicative formal group law.
And \lstinline|(R := R)| in the parentheses means that here we want the coefficient ring of the
formal group laws to be ring \lstinline|R|.

Therefore, we could deduce that any formal group law over a domain $R$ is commutative with a
neat proof.
