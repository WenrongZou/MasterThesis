...
In this section, we discuss some formalization of multivariate power series in mathlib.

Multivariate power series is defined to be a function from
\lstinline|(σ →₀ ℕ)| to \lstinline|R|, where \lstinline|(σ →₀ ℕ)| is a
finite supported function from index set \lstinline|σ| to natural number
\lstinline|ℕ| and \lstinline|R| is a ring.
...
The discussion of namespaces can be found in Section~\ref{sec: namespace}.
...
A special case of monomials is indeterminate indexed by \lstinline|s : σ|,
denoted as \lstinline|MvPowerSeries.X s|.
In mathlib, \lstinline|MvPowerSeries.X s|
...
is defined to be a ring homomorphism from a semiring \lstinline|R| to
\lstinline|MvPowerSeries σ R|. And its underlying function is
...
Another important operation on multivariate power series is \emph{formal
substitution}.  Suppose that \lstinline|f : MvPowerSeries σ R| is a power
...
, which records the condition under which the
...
fixed exponent \lstinline|d : τ →₀ ℕ|, only finitely many of the series
...
in mathlib.
If a function is finite supported, then
...
formal substitution.  Given a nonzero natural number \lstinline|p|,
